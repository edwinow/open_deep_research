...
async def generate_report_plan(state: ReportState, config: RunnableConfig):
    ...
    system_instructions_query = report_planner_query_writer_instructions.format(
        ...
        document_type=configurable.document_type,
        objective=configurable.objective,
    )
    ...
    results = structured_llm.invoke(
        [
            ...
            HumanMessage(
                content="Generate search queries that will help with planning the sections of the {}.".format(
                    configurable.document_type
                )
            ),
        ]
    )
    ...
    planner_message = """Generate the sections of the {}. Your response must include a 'sections' field containing a list of sections. 
                        Each section must have: name, description, plan, research, and content fields.""".format(
        configurable.document_type
    )
...
def write_section(
    ...
    2. Writes a section based on those results
    3. Evaluates if more research is needed
    4. Either returns to search or completes the section

    Args:
        state: Current state with search results
        config: Configuration for LLM models

    Returns:
        Command to either end this section or search for more info
    """
    ...
    # Format section writer instructions
    ...
    section.content = writer_model.invoke(
        [
    ...
    # Grade prompt
    section_grader_message = """Grade the {} and consider follow-up questions for missing information.
                               If the grade is 'pass', return empty strings for all follow-up queries.
                               If the grade is 'fail', provide specific search queries to gather missing information.""".format(
        configurable.document_type
    )
